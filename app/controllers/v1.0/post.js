...
//         else {
            ...
            if(post.type == 'group'){
                ...
                var latitude = req.param('latitude');
                post.location = [longitude, latitude];
                post.expire = req.param('expireTime');
                post.createdBy = req.param('createdBy');
            }
...
// 		});
    ...
    updateGroupImageUrl: function(req, res, next){
        if (
                _.isNull(req.param('groupImageUrl')) ||
                _.isUndefined(req.param('groupImageUrl'))
        ) {
            res.send(400, util.failure("groupImageUrl is required"));
        }
        else {
            ...
            .exec(function(err, post) {
                if (!err) {
                    if (!post) {
                        res.send(404, util.failure('Group not found'));
                        console.log('*********************');
                    }
                    else {
                        post.imageUrl.pictureUrl = req.param('groupImageUrl');
                        post.title = req.param('name');
                        post.message = req.param('description');
...
// 						});
    ...
    getAllGroups: function(req,res,next){
        ...
        .exec(function(err, posts) {
            ...
            if(err){
                res.send(400, util.failure("Unable to get my groups: " + err));
            }
            else{
                res.send(util.success({
                    groups: posts
    ...
    getAllEvents: function(req,res,next){
        ...
        .exec(function(err, posts) {
            ...
            if(err){
                res.send(400, util.failure("Unable to get my events: " + err));
            }
            else{
                for (i = 0; i < posts.length; i++)
                {
                    var expireDate = posts[i].expire;
                    var now = Date().now;
                    if (expireDate < now) {
                        Post.remove({ "_id": posts[i].id }, function(err) {});
                        posts.splice(i,1);
                    };
                }
    ...
    attendToEvent: function(req,res,next){
            Post.findOne({
                "_id": req.param('threadID')
            })
            .exec(function(err, post) {
                ...
                if(!err){
                    if(post){
                        ...
                        var attendToAdd = req.param('attendUser');
                
                        var membersCount = post.attends.length + 1;
                        
                        var j = post.attends.indexOf(attendToAdd);

                            if(j == -1){
                                post.attends.push(attendToAdd);
                            }
                            post.save(function(err,post){
                                if(!err){
    ...
    kickoutMember: function(req,res,next){
    
        var userToKickout = req.param('userIdToKickout');
        ...
        .exec(function(err, post) {
            if(err){
                ...
                if(!post){
                    ...
                    var i = post.members.indexOf(userToKickout);                  
                    ...
                    if(i != -1) {
                        ...
                        res.send(502, util.failure("Already Kickout"));
                    }
    ...
    getSpecGroup: function(req, res, next){
...
module.exports = {
    routes: [
        ...
        method: api.updateGroupImageUrl,
        verb: 'post',
        route: '/post/updateGroupImageUrl',
        middleware: [
            passport.authenticate('jwt', {
                session: false
            })
        ...
        method: api.getAllGroups,
        verb: 'get',
        route: '/post/getallgroups',
        middleware: [
            passport.authenticate('jwt', {
                session: false
            })
        ...
        method: api.getAllEvents,
        verb: 'get',
        route: '/post/getallevents',
        middleware: [
            passport.authenticate('jwt', {
                session: false
            })
        ...
        method: api.attendToEvent,
        verb: 'post',
        route: '/post/attendtoevent',
        middleware: [
            passport.authenticate('jwt', {
                session: false
            })
        ...
        method: api.kickoutMember,
        verb: 'post',
        route: '/post/kickoutmember'
    }, {
        ...
        method: api.getSpecGroup,
        verb: 'post',
        route: '/post/getspecgroup'
    }, {
